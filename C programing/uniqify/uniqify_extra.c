...
//Functions similar to uniqify.c except it uses two pipe line
//to seperate depending on its line number and merge for single output.
...
int main(int argc, char* argv[]){
	...
	if (child2 == 0){
		...
		close(pspipe[1]); 
		close(ps1pipe[0]);
		close(ps1pipe[1]);// close pipe parser to both sort
		close(sspipe[1]); 
		close(ss1pipe[1]);// close pipe write to both sort
...
void mergesuppressor(int file, int file1){ // merge two sort pipe into one.
	FILE *sstream[2];
	char nfile[2][1000];
	char cfile[31];
	int count = 0;
	...
	sd = (sd+1)%2;
	//compare last word of finished sort pipe and remaining sort pipe. 
	if(strlen(nfile[sd]) -1 < 31){
		if(strcmp(cfile, nfile[sd]) == 0){
			count++;
		}else if (count >= 1){
